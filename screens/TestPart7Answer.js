...
const TestPart7Answer = ({ navigation, route }) => {
    ...
    const fetchTestPartData = async () => {
      try {
        ...
        if (response.status === 200) {
          const part7Questions = response.data.groupQuestions
            .filter(group => group.part?.key === 'part7')
            .map(group => ({
              id: group.id,
              title: group.title,
              detail: group.detail,
          ...
          setQuestionData(part7Questions);
          setIsLoading(false);
        } else {
...
export default TestPart7Answer;
