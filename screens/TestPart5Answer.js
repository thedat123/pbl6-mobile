...
const TestPart5Answer = ({ navigation, route }) => {
    ...
    const fetchAnswerData = async () => {
      try {
        ...
        if (response.status === 200) {
          const part5Questions = response.data.groupQuestions
            .filter(group => group.part?.key === 'part5')
            .map(group => ({
              id: group.id,
...
export default TestPart5Answer;
