...
import { View, Text, Image, StyleSheet, SafeAreaView, ScrollView, ActivityIndicator, TouchableOpacity, Dimensions } from 'react-native';
...
const TestPart3Answer = ({ navigation, route }) => {
    ...
    const fetchAnswerData = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          const transformedAnswers = part3Questions.map(group => ({
            id: group.id,
  ...
  if (isLoading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading test answers...</Text>
      </View>
    );
  }
...
export default TestPart3Answer;